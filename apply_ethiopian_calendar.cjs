...
function processFile(filePath) {
  ...
  if (content.includes('toLocaleDateString')) {
    if (!content.includes('formatToEthiopian')) {
      ...
      let relativePath = depth <= 0 ? './utils/ethiopianDate' : '../'.repeat(depth) + 'utils/ethiopianDate';
    ...
    content = content.replace(/([a-zA-Z0-9_]+)\.toLocaleDateString\([^)]*\)/g, 'formatToEthiopian($1)');
  }
  ...
  if (content.includes('type="date"') && !filePath.includes('EtDatePicker.jsx')) {
    if (!content.includes('EtDatePicker')) {
      const depth = filePath.replace(srcDir, '').split(path.sep).length - 2;
      let relativePath = depth <= 0 ? './components/common/EtDatePicker' : '../'.repeat(depth) + 'components/common/EtDatePicker';
