      ...
      {sortedKeys.length === 0 ? (
        ...
        sortedKeys.map((key) => {
          ...
          return (
            <div
              key={key}
              className="bg-surface-container-lowest rounded-xl sm:rounded-2xl overflow-hidden border border-outline-variant/20 shadow-sm"
            >
              <div
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <h3 className="text-sm sm:hidden font-bold text-on-surface truncate">
                        {displayDate}
                      </h3>
                      <h3 className="text-xl hidden sm:block font-bold text-on-surface">
                        {displayDateLong}
                      </h3>
              ...
              {isExpanded && (
                <div className="divide-y divide-outline-variant/10">
                  {records.map((record) => (
                    <div
                      key={record.id}
                      className="p-3 sm:p-4 flex items-center justify-between hover:bg-surface-container-low transition-colors"
                    >
                      <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs sm:text-sm shrink-0 overflow-hidden">
                          {record.members?.image_url ? (
                            <img
                              src={record.members.image_url}
                              alt={record.members?.full_name}
                      ...
                      <span
                        className={`px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-full text-[9px] sm:text-xs font-bold tracking-wide uppercase shrink-0 ${
