    ...
    <div className="min-h-screen bg-surface text-on-surface selection:bg-tertiary-fixed-dim/30 font-sans overflow-x-hidden">
          ...
          <div className={`relative px-8 h-20 rounded-2xl flex items-center justify-between transition-all duration-500 ${scrolled ? 'bg-surface/80 backdrop-blur-xl shadow-whisper border border-primary/5' : 'bg-transparent'}`}>
            ...
            <div className="hidden md:flex items-center gap-10">
              ...
              <div className="flex items-center gap-6">
                <Link to="/login" className="label-sm font-black uppercase tracking-widest text-primary hover:text-tertiary-fixed-dim transition-colors flex items-center gap-2 cursor-pointer">
                  Sign In <ChevronRight size={14} />
                </Link>
                <Link to="/login">
                  <Button className="bg-primary hover:bg-tertiary-fixed-dim text-on-primary hover:text-primary rounded-xl px-8 py-3 h-auto font-black uppercase tracking-widest text-[10px] transition-all duration-500">
                    Get Started
                  </Button>
                </Link>
              </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <p className="text-[10px] font-black uppercase tracking-widest text-primary/30 mb-2">Navigation</p>
            <div className="grid grid-cols-1 gap-2">
              <Link to="/about" className="p-4 bg-primary/5 rounded-2xl text-primary font-bold flex justify-between items-center">Vision <span>&rarr;</span></Link>
              <Link to="/mission" className="p-4 bg-tertiary-fixed-dim/5 rounded-2xl text-primary font-bold flex justify-between items-center">Mission <span>&rarr;</span></Link>
              <Link to="/community" className="p-4 bg-primary/5 rounded-2xl text-primary font-bold flex justify-between items-center">Groups <span>&rarr;</span></Link>
            </div>
            <div className="h-[1px] bg-primary/10 my-2"></div>
            <Link to="/login" className="w-full">
              <Button onClick={() => setIsMenuOpen(false)} className="w-full bg-primary py-6 rounded-2xl h-auto font-black uppercase tracking-widest text-[11px] text-on-primary">Sign In to Dashboard</Button>
            </Link>
          </motion.div>
        )}
      </AnimatePresence>
