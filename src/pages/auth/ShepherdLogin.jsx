...
import { Mail, Lock, ArrowRight, ChevronLeft, Eye } from "lucide-react";

export default function ShepherdLogin() {
      ...
      >
        <div className="text-center mb-16">
          <div className="w-24 h-24 rounded-[3rem] bg-primary grid place-items-center mx-auto mb-8 shadow-xl shadow-primary/10">
            <Eye size={48} className="text-on-primary" />
          </div>
          <h1 className="display-sm text-primary mb-4">
            Shepherd<span className="text-tertiary-fixed-dim italic"> Portal</span>
          </h1>
          <p className="label-sm opacity-60 uppercase tracking-widest">
            Group Oversight & Supervisory Access
          </p>
        </div>
        ...
        <Card className="p-12 md:p-16 shadow-whisper border-none bg-surface-container">
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            <Input
              label="Shepherd Email"
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              required
              icon={Mail}
              placeholder="shepherd@gmkc-church.org"
            />
            <Input
              label="Password"
              type="password"
            ...
            <Button type="submit" loading={loading} className="h-16 bg-primary hover:opacity-90 text-on-primary border-none shadow-xl shadow-primary/10">
              Sign In to Shepherd Portal <ArrowRight size={20} className="ml-3" />
            </Button>
          </form>
        </Card>
