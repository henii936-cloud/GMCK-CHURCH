...
const FOLLOW_UP_STATUSES = ["Pending", "Contacted", "Integrated", "Lost Contact"];
const followupColor = {
...
const emptyForm = { full_name: "", phone: "", address: "", gender: "Male", conversion_date: "", follow_up_status: "Pending", notes: "" };
...
export default function Converts() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const { data } = await supabase.from("new_converts").select("*").order("conversion_date", { ascending: false });
    setConverts(data || []);
    setLoading(false);
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm("Delete this convert record?")) return;
    await supabase.from("new_converts").delete().eq("id", id);
    fetchData();
  };
  ...
  const handleSave = async (e) => {
    ...
    if (editItem) { await supabase.from("new_converts").update(payload).eq("id", editItem.id); }
    else { await supabase.from("new_converts").insert(payload); }
    setSaving(false);
    setShowModal(false);
    fetchData();
  };
  ...
  return (
    <div className="animate-fade-in max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          <h1 className="display-sm text-primary">New <span className="text-tertiary-fixed-dim italic">Converts</span></h1>
          <p className="text-on-surface-variant text-sm mt-1">Track new believers and follow-up activities</p>
        </div>
      ...
      <div className="flex gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search converts..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div className="space-y-3">
          {filtered.map((c, i) => (
            <motion.div key={c.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="w-11 h-11 rounded-2xl bg-amber-500/10 flex items-center justify-center flex-shrink-0 font-black text-amber-600 text-lg">
                {c.full_name?.charAt(0)}
              </div>
              <div className="flex-1 overflow-hidden">
                <p className="font-bold text-sm text-primary">{c.full_name}</p>
                <p className="text-xs text-on-surface-variant">{c.gender} · {c.phone || "No phone"} · Converted: {formatToEthiopian(c.conversion_date)}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${followupColor[c.follow_up_status]}`}>{c.follow_up_status}</span>
                <button onClick={() => openEdit(c)} className="w-9 h-9 rounded-xl hover:bg-surface-container-high flex items-center justify-center text-on-surface-variant hover:text-primary transition-all"><Edit2 size={14} /></button>
                <button onClick={() => handleDelete(c.id)} className="w-9 h-9 rounded-xl hover:bg-red-500/10 flex items-center justify-center text-on-surface-variant hover:text-red-500 transition-all"><Trash2 size={14} /></button>
              </div>
            </motion.div>
          ))}
          {filtered.length === 0 && (
            <div className="text-center py-20 text-on-surface-variant/40">
              ...
              <p className="font-medium">No converts found.</p>
            </div>
          )}
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex justify-between items-center mb-6">
                <h2 className="headline-sm font-black text-primary">{editItem ? "Edit Convert" : "Add New Convert"}</h2>
              ...
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="label-sm text-on-surface-variant font-black uppercase tracking-widest text-[10px] mb-2 block">Gender</label>
                ...
                <div>
                  <label className="label-sm text-on-surface-variant font-black uppercase tracking-widest text-[10px] mb-2 block">Address</label>
                  <input value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))}
                    className="w-full h-12 rounded-xl bg-surface-container-low border border-outline-variant/20 px-4 text-sm text-primary outline-none" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="label-sm text-on-surface-variant font-black uppercase tracking-widest text-[10px] mb-2 block">Conversion Date</label>
                    <input type="date" value={form.conversion_date} onChange={e => setForm(f => ({ ...f, conversion_date: e.target.value }))}
                      className="w-full h-12 rounded-xl bg-surface-container-low border border-outline-variant/20 px-4 text-sm text-primary outline-none" />
                  </div>
                  <div>
                    <label className="label-sm text-on-surface-variant font-black uppercase tracking-widest text-[10px] mb-2 block">Follow-up Status</label>
                    <select value={form.follow_up_status} onChange={e => setForm(f => ({ ...f, follow_up_status: e.target.value }))}
                      className="w-full h-12 rounded-xl bg-surface-container-low border border-outline-variant/20 px-4 text-sm text-primary outline-none">
                      {FOLLOW_UP_STATUSES.map(s => <option key={s}>{s}</option>)}
                    </select>
                  </div>
