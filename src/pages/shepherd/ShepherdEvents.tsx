    ...
    <div className="animate-fade-in max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
        <div>
          ...
          <p className="label-sm text-on-surface-variant tracking-widest">
            Plan shepherd-led ministry events and meetings.
          </p>
        </div>
        <Button
          onClick={openCreate}
          className="bg-primary text-on-primary border-none h-12 px-6"
        >
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-3 rounded-2xl bg-surface-container-high text-on-surface-variant hover:text-primary transition-colors"
                  >
                    <Edit2 size={18} />
                  </button>
                  <button
                    onClick={() => handleDelete(evt.id)}
                    className="p-3 rounded-2xl bg-surface-container-high text-on-surface-variant hover:text-red-500 transition-colors"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
              ...
              <div className="space-y-1 mb-5 text-sm text-on-surface-variant">
                <p className="flex items-center gap-2">
                  <Clock size={12} /> {evt.event_time || "Time not set"}
                </p>
                <p className="flex items-center gap-2">
                  <MapPin size={12} /> {evt.location || "Location not set"}
                </p>
                <p>📅 {formatToEthiopian(evt.date)}</p>
                <span className="inline-flex px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.25em] bg-surface-container-high text-on-surface-variant">
                  {evt.status || "Upcoming"}
                </span>
              </div>
            </motion.div>
          ...
          {filteredEvents.length === 0 && (
            <div className="col-span-full text-center py-20 text-on-surface-variant/50">
              ...
              <p className="font-medium">
                No shepherd events yet. Start planning one now.
              </p>
            </div>
          )}
        </div>
          ...
          <>
            ...
            >
              <Card className="w-full max-w-2xl p-10 bg-surface-container border-none shadow-2xl relative my-4">
                <button
                  onClick={() => setModalOpen(false)}
                  className="absolute top-4 right-4 w-9 h-9 rounded-xl bg-surface-container-low flex items-center justify-center text-on-surface-variant hover:text-primary"
                >
                  <X size={18} />
                </button>
                <h2 className="headline-sm text-primary font-black mb-8">
                  {editId ? "Edit Shepherd Event" : "Create Shepherd Event"}
                </h2>
